    ...
    <table id="example1" class="table table-bordered table-striped">
     ...
     ?>
      <tr>
        ...
        <td>
            <?php
            if($row->verif_kabalai==0){
            ?>
            <a class='btn btn-success btn-xs' title='Edit' href="<?php echo base_url() ?>sijuara/buat_surat_masuk?id_sm=<?php echo $row->id_surat_masuk ?>"><i class='fas fa-edit'></i> Edit</a>
            <a class='btn btn-primary btn-xs' title='Copy' href="<?php echo base_url() ?>sijuara/buat_surat_masuk?copy=<?php echo $row->id_surat_masuk ?>"><i class='fas fa-copy'></i> Copy</a>
            <a class='btn btn-danger btn-xs' title='Delete Data' href="<?php echo base_url() ?>sijuara/delete_surat_masuk/<?php echo $row->id_surat_masuk ?>" onclick="return confirm('Apa anda yakin untuk hapus Data ini?')"><i class='fa fa-trash'></i> Hapus</a>
            ...
            ?>
            <a class='btn btn-success btn-xs' title='Detail' href="<?php echo base_url() ?>sijuara/sm_detail/<?php echo $row->id_surat_masuk ?>/<?= $uri3 ?>"><i class='fas fa-file'></i> Detail</a>
            <?php
            }
            
            if(!empty($row->file_pdf)){
            ?>
            <a class='btn btn-warning btn-xs' target='_blank' title='PDF' href="<?php echo base_url() ?>asset/file_lainnya/surat_masuk/<?php echo $row->file_pdf ?>"><i class='fas fa-file-pdf'></i> PDF</a>
            <?php
            }
            ?>
        </td>
