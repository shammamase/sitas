...
<?php
    $get_varietas_akabi = $this->db->query("select a.*,c.nama_lengkap
                                          from cltr_post a
...
<div id="accordion">
    <?php
        $nor = 1;
        foreach($get_varietas_akabi->result() as $gva){
            if($nor==1){
                $sh = "show";
            } else {
                $sh = "";
    ...
    <div class="card">
      ...
      <div id="collapse<?php echo $nor ?>" class="collapse <?php echo $sh ?>" data-parent="#accordion">
        <div class="card-body">
          <img class="img-fluid" src="<?php echo base_url() ?>asset/foto_content/<?php echo $gva->gambar ?>">
          <?php echo $gva->isi_berita ?>
          <b>Penulis : <?php echo $gva->nama_lengkap ?></b>
        </div>
      </div>
    </div>
    <?php
